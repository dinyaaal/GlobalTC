(() => {
    ...
    function menuInit() {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            ...
            } else if (document.documentElement.classList.contains("menu-open") && bodyLockStatus && !e.target.closest(".menu__wrapper")) {
                bodyLockToggle();
                document.documentElement.classList.toggle("menu-open");
    ...
    document.addEventListener("DOMContentLoaded", (function() {
        ...
        selectLangs.forEach((function(selectLang) {
            const langTitleBody = selectLang.querySelector(".select-lang__title");
            langTitleBody.querySelector("p");
            const langBody = selectLang.querySelector(".select-lang__body");
            langTitleBody.addEventListener("click", (function() {
                langBody.classList.toggle("active");
            }));
            document.addEventListener("click", (function(event) {
                if (!selectLang.contains(event.target)) langBody.classList.remove("active");
            }));
        }));
    }));
    function ticker(id, speed) {
        const carousels = document.querySelectorAll(`.js-carousel`);
        const mediaQuery = window.matchMedia("(max-width: 768px)");
        if (mediaQuery.matches) speed /= 1.5;
        carousels.forEach(((carousel, index) => {
            const parentSelector = carousel;
            const clone = parentSelector.innerHTML;
            const firstElement = parentSelector.children[0];
            let i = 0;
            animateCarousel("beforeend", "left", firstElement, clone, parentSelector, i, speed);
        }));
    }
    function animateCarousel(position, margin, element, clone, parentSelector, i, speed) {
        parentSelector.insertAdjacentHTML(position, clone);
        parentSelector.insertAdjacentHTML(position, clone);
        setInterval((function() {
            margin === "left" ? element.style.marginLeft = `-${i}px` : element.style.marginRight = `-${i}px`;
            if (i > element.clientWidth) i = 0;
            i += speed;
        }), 0);
    }
    ticker("my-carousel", .3);
    window["FLS"] = 0;
    isWebp();
    addLoadedClass();
    menuInit();
